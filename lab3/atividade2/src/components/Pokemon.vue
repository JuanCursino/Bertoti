...
export default {
  ...
  methods: {
    ...
    async saveClient()
    {
      ...
      {
        try {
          await axios.put(`http://localhost:8080/pokemons/${this.client.id}`, { name: this.client.name })
          this.clients[this.clientIdx] = this.client
          this.showSnackbar("pokemonsupdated successfully", "green")
        } catch(e) {
          console.error(`Failed to edit client (${e})`)
          this.showSnackbar("Failed to update the pokemons info", "red")
        }
      } else {
        try {
          const response = await axios.post("http://localhost:8080/pokemons", { name: this.client.name })
          this.clients.unshift(response.data)
          this.client = response.data
          this.showSnackbar("pokemons saved successfully", "green")
          this.isUpdateMode = true
        } catch(e) {
          console.error(`Failed to save pokemons (${e})`)
          this.showSnackbar("Failed to save the pokemons", "red")
        }
      }
    },
    createClient()
    ...
    {
      try {
        await axios.delete(`http://localhost:8080/pokemons/${this.client.id}`)
        this.clients.splice(this.clientIdx, 1)
        this.clientDeleteDialog = false
        this.showSnackbar("pokemons removed successfully", "green")
      } catch (e) {
        console.error(`Failed to delete pokemons (${e})`)
        this.showSnackbar("Failed to remove the pokemons", "red")
      }
    },
